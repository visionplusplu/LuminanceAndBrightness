...
for Subject = 1:ValidSubjectNumber
    ...
    %%%% One patch wasn't tested, so we interpolated the rankings of it %%%%
...
if strcmp(path,'LaboratoryTest')
    figure('name','Fig. 1B','NumberTitle','off');
    hold on
    for nInt = 1:12
...
if strcmp(path,'LaboratoryTest')
    if exist(['./Results/nonChromaVariationExperiment/',path,'/WeightFitting.mat']) ~=2
        WeightFitting = FitWeights(cubeRGBs4Fitting,MeanRanking,AllSub.PatchRanking_allParticipant_InterPolate,RankingPlotTitle,"Fig. 2B");
        ...
        WeightHeatMap(['Max-Weighted Map (',RankingPlotTitle,')'],Allweight,WeightList,WeightMat,OptimalWeight_individual,OptimalWeight_mean,RGB_weights_rad,RGB_weights_lum,"Fig. 2B");
    end
else
    if exist(['./Results/nonChromaVariationExperiment/',path,'/WeightFitting.mat']) ~=2
...
if strcmp(path,'LaboratoryTest')
    if exist(['./Results/nonChromaVariationExperiment/',path,'/WeightFitting_DKL.mat']) ~=2
        load(['./ColorPatchMetric/BrightnessModel_144Patches.mat']);
        WeightFitting_DKL = FitWeights(BrightnessModel.NormalizedDKL,MeanRanking,AllSub.PatchRanking_allParticipant_InterPolate,RankingPlotTitle,[]);
        save (['./Results/nonChromaVariationExperiment/',path,'/WeightFitting_DKL.mat'],"WeightFitting_DKL");
    else
        load(['./Results/nonChromaVariationExperiment/',path,'/WeightFitting_DKL.mat']);
    end
end

if strcmp(path,'LaboratoryTest')
    if exist(['./Results/nonChromaVariationExperiment/',path,'/WeightFitting_LMS.mat']) ~=2
        load(['./ColorPatchMetric/BrightnessModel_144Patches.mat']);
        WeightFitting_LMS = FitWeights(BrightnessModel.Normalizedlms,MeanRanking,AllSub.PatchRanking_allParticipant_InterPolate,RankingPlotTitle,[]);
        save (['./Results/nonChromaVariationExperiment/',path,'/WeightFitting_LMS.mat'],"WeightFitting_LMS");
    else
        load(['./Results/nonChromaVariationExperiment/',path,'/WeightFitting_DKL.mat']);
    end
end
...
if strcmp(path,'LaboratoryTest')
    FitExponent(cubeRGBs4Fitting,MeanRanking,WeightFitting.meanFit.OptimalMaxWeight,'Fig. 2A');
end

%% Comparison across Displays
if strcmp(path,'LaboratoryTest')
    caliPath = ['../RawData/nonChromaVariationExperiment/Displays/'];
    DisplayComparison(corrRGB,caliPath,oled_weights,cubeRGBs4Fitting,WeightFitting.meanFit.OptimalSumWeight,WeightFitting.meanFit.OptimalMaxWeight,AllSub.Name,"fig. S2A","fig. S2C","fig. S2E");
end
