function WeightHeatMap(WhichWeight,Allweight,WeightList,WeightMat,OptimalWeight_individual,OptimalWeight_mean,RGB_weights_rad,RGB_weights_lum, FigId)
if nargin < 1 || isempty(FigId)
    FigId = "";
end
FigId = char(FigId);
...
%% Plot
f = figure("Name",FigId,'NumberTitle','off');hold on;
...
a = colorbar;
a.Direction = 'reverse';
a.Ticks = [0 0.2 0.4 0.6 0.8 1];
a.TickLabels = {'100','80','60','40','20','0'};
a.Label.Position(1) = 3;

title("Unexplained Variance (%)");
legend({['Optimal Fit wMaxRGB' newline  '(Mean Rankings)'],'Luminance','Radiance',['Optimal Fit wMaxRGB' newline  '(Individual Rankings)']},'NumColumns',1,'FontSize',8);
xlabel('wB/wR');
ylabel('wG/wR');
xlim([0,3]);
ylim([0,4]);
